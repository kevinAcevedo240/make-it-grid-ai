...
// Enhance ResponsiveGridLayout with WidthProvider to handle responsive layouts
const ResponsiveGridLayout = WidthProvider(Responsive);

const GridLayout: React.FC = () => {

  // Accessing grid settings and actions from GridContext
  const {
    rows,
    cols,
    layout,
  ...
  // Media query to detect if the screen size is for desktop or laptop
  const isDesktopOrLaptop = useMediaQuery({ minWidth: 768 });

  // Breakpoints and columns configuration for different screen sizes
  ...
  return (
    <div className="mt-8">
      ...
      {/* Render GridSettings component to allow users to modify grid settings (Cols, Rows, Gap) */}
      <GridSettings/>
      ...
      >
        ...
        >
          {/* Render grid cells if columns and rows are greater than 0 */}
          {cols > 0 && rows > 0 && (
            <div
              ...
              {Array.from({ length: rows * cols }).map((_, index) => {
                ...
                return (
                  <div
                    ...
                    style={{ zIndex: 1 }}
                  >
                    <div className={`absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-primary text-white rounded-full ${(isMobile || !isDesktopOrLaptop) && cols >= 8 ? "size-5" : "size-6 sm:size-8"} flex items-center justify-center  z-1`}>
          ...
          {/* Render the responsive grid layout if columns are greater than 0 */}
          {cols > 0 && (
      ...
      {/* Add spacing at the bottom for mobile devices */}
